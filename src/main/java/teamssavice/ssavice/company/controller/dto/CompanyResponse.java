...
public class CompanyResponse {
    ...
    public record MyCompany(
        ...
        public static CompanyResponse.MyCompany from(CompanyModel.MyCompany model) {
            List<ServiceItemResponse.Summary> services = model.service().stream()
                .map(ServiceItemResponse.Summary::from).toList();
    ...
    public record Info(
        ...
        public static CompanyResponse.Info from(CompanyModel.Info model) {
            List<ServiceItemResponse.Summary> services = model.myCompany().service().stream()
                .map(ServiceItemResponse.Summary::from).toList();
            List<ReviewResponse.Item> reviews = model.review().stream()
                .map(ReviewResponse.Item::from).toList();
            return Info.builder()

...
public class ServiceItemReadService {
    ...
    @Transactional(readOnly = true)
    public Page<ServiceItem> findByCompany(ServiceItemCommand.RetrieveByCompany command) {
        return serviceItemRepository.findByCompany(command.companyId(), command.status(), command.pageable());
    }
}
